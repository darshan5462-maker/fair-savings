...
export default function AdminLoanGivingPage() {
  const { t } = useLanguage();
  ...
  function load() {
    setLoading(true);
    Promise.all([api.get("/payer-loans"), api.get("/members")])
      .then(([loansRes, membersRes]) => {
        setLoans(loansRes.data.data);
        // A loan can only be given by a payer/standalone member, not a
        // member who is themselves someone else's linked child.
  ...
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault();
    if (!form.payerId) return toast.error("Select a payer");
    setSubmitting(true);
    try {
      const payload: any = { payerId: form.payerId, borrowerType: form.borrowerType, principalAmount: form.principalAmount };
      ...
      setShowAdd(false);
      setForm({ payerId: "", borrowerType: "SELF", principalAmount: 1000, name: "", phone: "" });
      load();
    } catch (err: any) {
            ...
            return (
              <div key={loan.id} className="glass-card p-5">
                <div className="mb-1 text-xs text-ink-500">
                  Payer: <span className="font-medium text-ink-900 dark:text-white">{loan.payer.name}</span>{" "}
                  <span className="font-mono">{loan.payer.username}</span>
                </div>
