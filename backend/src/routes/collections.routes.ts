...
/**
 * GET /api/collections/upcoming-dates?startWeek=1&weeks=4
 * Due dates for a specific range of week numbers, computed the same way as
 * the individual schedule endpoint - as a direct function of week number
 * and the shared scheme start date. NOT a scan from "today's" calendar:
 * that approach breaks the moment a family pays ahead of schedule (e.g. two
 * weeks recorded in one sitting), because "today" no longer lines up with
 * whatever week number they're actually due for next.
 */
router.get("/upcoming-dates", async (req, res) => {
  const weeksAhead = Math.min(52, Number(req.query.weeks) || 4);
  const startWeek = Math.max(1, Number(req.query.startWeek) || 1);
  ...
  for (let i = 0; i < weeksAhead; i++) {
    dates.push(collectionDueDate(startDate, startWeek + i, collectionDay));
  }
